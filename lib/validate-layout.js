...
export function validateLayout(layout) {
  ...
  // 5. SLOT-COUNT TRUTH — declared count matches reality; strip never 1.
  if (layout.slots.length !== layout.slotCount) {
    violations.push(`slotCount=${layout.slotCount} but ${layout.slots.length} slots defined`);
  }
  if (layout.format === "strip" && layout.slotCount < 2) {
    violations.push(`strip must have ≥2 slots (has ${layout.slotCount})`);
  }
