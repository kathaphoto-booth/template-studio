...
export function VaultDrawer({ selection, openDates, onClose }: VaultDrawerProps) {
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const rawHash = json.lead_hash || crypto.randomUUID();
      const token = `KB-${rawHash.slice(0, 7).toUpperCase()}`;
                ...
                <div className="flex flex-col gap-6">
                  ...
                  >
                    <div
                      ...
                      aria-hidden="true"
                    />
                    <p className="font-mono text-[9px] tracking-[0.24em] uppercase" style={{ color: 'var(--color-katha-fnt)' }}>
                    ...
                    <div className="border-t border-dashed pt-5 flex flex-col gap-2" style={{ borderColor: 'rgba(17,15,13,0.25)' }}>
                      ...
                      <p className="font-mono text-[10px] tracking-[0.16em] uppercase" style={{ color: 'var(--color-katha-fnt)' }}>
                        {receipt.tier}
                      </p>
                    </div>
                    <p className="font-mono text-[9px] tracking-[0.1em] mt-6 break-all" style={{ color: 'var(--color-katha-fnt)' }}>
                  ...
                  {content.integrations.HONEYBOOK_LEAD_FORM.startsWith('http') ? (
                    <div className="w-full bg-[var(--color-katha-l2)] rounded-[2px] overflow-hidden border border-[var(--color-katha-ln)] mt-2">
                      <iframe
                        src={new URL(content.integrations.HONEYBOOK_LEAD_FORM).toString()}
                    ...
                    <div className="border border-[var(--color-katha-ln2)] p-5 rounded-[2px] mt-2">
                      <p className="font-mono text-[9px] tracking-[0.14em] uppercase text-[var(--color-katha-mut)]">
                        A deposit invoice and contract follow via HoneyBook — watch your inbox.
                      </p>
                    </div>
                  )}
